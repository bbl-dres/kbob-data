...
| Fusszeile behauptete «kein publizierter Stand» auch bei fremdem Endpunkt; kein Abfragezeitpunkt. | Umgesetzt: INT-Satz nur beim INT-Endpunkt, sonst neutral «Daten von …»; «Abgefragt am …» steht jetzt in der Fusszeile und das Datum im CSV-Dateinamen. |
...
| **Ladevorgänge waren für Screenreader stumm** (Spinner ohne Live-Region, kein `aria-busy`). | Umgesetzt: `busy()`-Texte laufen über die `#status`-Live-Region; der Ladering dreht sichtbar im Inhaltsbereich (Tabelle/Galerie/Graf) statt im Kopf. |
