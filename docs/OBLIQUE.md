...
| Hauptnavigation | `.ob-master-layout-navigation` > `nav` > `ul.ob-main-nav` > `li.ob-main-nav-item` > `a.ob-master-layout-navigation-link.ob-main-nav-link` (+ `.active`, `aria-current="page"`); zwei Einträge nebeneinander: Katalog (`#`), Anleitung (`#anleitung`); erste Ebene expanded: 12/16-px-Polster, 3-px-Unterkante, Hover `secondary-50`, aktiv/hover Akzentrot `$ob-accent` | master-layout-navigation.component |
| Export-Knopf | `li.ob-service-navigation-custom-control` > `button.ob-button.ob-button-secondary` | dito + Button-Tokens |
| Info-Widget («Hilfe und Kontakt») | `li.ob-service-navigation-info` > `button#info-toggle.ob-button.ob-button-tertiary.ob-icon-button` (Icon `question`) + `div.ob-popover-content[role=dialog]` > `.ob-popover-arrow` + `.ob-popover-content-wrapper` > Beschreibung, `section.ob-service-navigation-popover-section` («Hilfe»: `ul.ob-popover-list > a.ob-popover-link`; «Kontakt»: `ul.ob-service-navigation-info-links`, Mail-Icon); Widget-Reihenfolge wie im CD: Custom-Controls → Info → Sprache | service-navigation/info, shared/popover-section, core/_popover.scss |
...
| Karten (Galerie) | `.ob-card` (Rand `#dfe4e9`, `shadow-lg`, hover `shadow-xl` + Fläche/Rand), drei Blöcke Kopf → Inhalt (`.app-card-body`: Beschreibung + Tags) → Signatur (`.app-card-footer` = `ob-signature`, Caption ohne Linie) je 16 px; Titelknopf mit gedehnter Klickfläche bleibt | material/_mat-card.scss |
